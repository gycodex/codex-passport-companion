...
4. 点击 **连接 Wi-Fi**。设备重启后连接路由器，不需要下载或转发配对文件。
5. 在同一路由器的电脑上打开 **Passport.exe**，选择「局域网 / Wi-Fi」→「搜索局域网设备」→选中设备→配对。
6. 核对电脑与设备屏幕的六位号码。只有号码一致，才在设备按 **OK**，并点击电脑上的 **号码一致，确认配对**。不一致时取消（设备按 **UP**）。30 秒内未完成会自动取消；重新尝试需距上次开始至少 60 秒。
7. 配对成功后密钥自动保存在本机，立即尝试连接。以后直接打开程序；IP 改变时会按设备标识重新发现，并用已有密钥验证身份。

此流程需要本次新增自动配对功能的固件和桌面端。旧版固件仍可在电脑的兼容选项中填写 IP、导入 `passport-lan.json`；手机配网页的「旧版电脑端兼容」保留文件下载。命令行用户仍可用 `--lan-key-file` 指定密钥文件。

热点只在主动配网时开启，10 分钟后自动关闭；设备配网页按确认键也可立即退出。未保存时保留原网络和连接模式；保存后退出会尝试连接新网络。若一直显示「连接中…」，重新进入配网检查密码。换 Wi-Fi 会保留已有配对密钥，电脑无需重新配对。
...
新电脑可重新搜索设备并完成双端确认，无需复制文件。自动配对保留已有设备密钥，因此原电脑仍可连接。
...
推荐直接在设备「设置」中开启蓝牙，Wi-Fi 会自动退出；再开启 Wi-Fi 即可使用原配网信息切回。切换自动重启一次，不清除配对信息。网页也需选择对应模式。
