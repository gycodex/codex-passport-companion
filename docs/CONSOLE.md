...
当前源码的 Windows 启动入口已改为独立桌面窗口，提供托盘和首次连接引导，见
[Windows 桌面版](DESKTOP.md)。下文浏览器行为可通过 `start-browser.cmd` 使用；
已发布的 v0.2.2 包仍是浏览器版。
...
使用单文件 `Passport.exe` 时无需安装 Python；运行要求见 [桌面版说明](DESKTOP.md)。

先安装 **Python 3.10+**（Windows 建议 64 位 Python 3.12）、Codex CLI，并在当前用户下登录 Codex。

| 系统 | 启动方式 |
| --- | --- |
| Windows 当前源码 | 双击 `start-console.vbs` 打开桌面版；`start-browser.cmd` 打开网页版 |
| macOS | 终端执行 `bash start-console.command`，允许系统蓝牙权限，保持终端运行 |

首次启动自动创建 `.venv-console` 并安装依赖，需要联网；以后会复用环境。桌面版重复启动会恢复已有窗口，网页版会打开已有后台的网页。Codex 路径默认优先从当前运行的 CLI 进程查找，通常不必填写。

仅使用控制台管理已有功能，不需要专门刷固件；使用局域网自动配对、蓝牙语音、首页录音等新增功能，需要本分支新版固件，见 [编译与烧录](../README.zh_CN.md#编译与烧录)。
...
| 局域网 | 按 [配网说明](LAN.md) 接入 2.4 GHz Wi-Fi | 选「局域网 / Wi-Fi」→搜索设备→选择配对→核对号码并在两端确认（需新固件） |
...
- **断线重连**：最多尝试三次；仍连不上会停止并显示错误，检查设备后点击「连接设备」可手动重试。
- **提醒检查 → 播放提醒**：测试设备完成提示，遵循设备音量和夜间静音设置。
- **退出**：尝试断开设备并停止后台；即使断开超时，退出也会继续。桌面版关闭窗口，浏览器版会尝试关闭网页；浏览器不允许自动关页时可手动关闭。
- 直接关闭网页不会停止后台。
...
新电脑重新安装依赖和虚拟音频驱动；不要复制 `.venv-console`。蓝牙重新配对，局域网重新搜索并在两端确认配对；输入法快捷键和音频设备需在新电脑重新选择。
