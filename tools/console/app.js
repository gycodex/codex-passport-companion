...
function configuration() { const cfg={mode:mode(),host:$('host').value,device:$('device').value,port:Number($('port').value),codex:$('codex').value,codex_auto:$('codex-auto').checked,autoconnect:$('autoconnect').checked,voice_enabled:$('voice-enabled').checked,voice_output:$('voice-output').value,voice_ime:$('voice-ime').value,voice_hotkeys:$('voice-hotkeys').checked,voice_start_key:$('voice-start-key').value,voice_stop_key:$('voice-stop-key').value}; if(pairing)cfg.pairing=pairing; return cfg; }
...
async function refresh(){const state=await api('status');if(!initialized){for(const id of ['host','device','port','codex'])$(id).value=state.config[id];$('autoconnect').checked=state.config.autoconnect;$('codex-auto').checked=state.config.codex_auto!==false;$('codex-manual').hidden=$('codex-auto').checked;document.querySelector(`input[name=mode][value="${state.config.mode}"]`).checked=true;toggleMode();
for(const field of ['enabled','hotkeys'])$('voice-'+field).checked=Boolean(state.config['voice_'+field]);
...
initialized=true;}
const voice=state.voice||{}; const voiceLabels={off:'语音输入未启用',ready:state.config.voice_output==='meter'?'麦克风测试已就绪，不触发输入法':'麦克风就绪，首页短按下键开始',recording:'正在录音，再次短按下键结束',error:voice.message||'语音连接异常'};
...
$('voice-ime').onchange=()=>{const preset={xunfei:'f6',doubao:'ctrl+shift+f8'}[$('voice-ime').value]||'';$('voice-start-key').value=preset;$('voice-stop-key').value=preset;notice(preset?'已填入快捷键预设，请确认与输入法设置一致，再保存并连接。':'请填写所选输入法的开始和结束快捷键。');};
