...
initialized=true;}
const voice=state.voice||{}; const voiceLabels={off:'语音输入未启用',ready:state.config.voice_output==='meter'?'麦克风测试已就绪，不触发输入法':'麦克风就绪，请在设备上按确认键开始',recording:'正在录音，再按设备确认键结束',error:voice.message||'语音连接异常'};
...
$('voice-ime').onchange=()=>{const xunfei=$('voice-ime').value==='xunfei';$('voice-start-key').value=xunfei?'f6':'';$('voice-stop-key').value=xunfei?'f6':'';notice('请核对所选输入法的开始和结束快捷键。');};
