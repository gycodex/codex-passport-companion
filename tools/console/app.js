...
function configuration() { const cfg={mode:mode(),host:$('host').value,device:$('device').value,port:Number($('port').value),codex:$('codex').value,codex_auto:$('codex-auto').checked,autoconnect:$('autoconnect').checked}; if(pairing)cfg.pairing=pairing; return cfg; }
...
async function refresh(){const state=await api('status');if(!initialized){for(const id of ['host','device','port','codex'])$(id).value=state.config[id];$('autoconnect').checked=state.config.autoconnect;$('codex-auto').checked=state.config.codex_auto!==false;$('codex-manual').hidden=$('codex-auto').checked;document.querySelector(`input[name=mode][value="${state.config.mode}"]`).checked=true;toggleMode();initialized=true;}
